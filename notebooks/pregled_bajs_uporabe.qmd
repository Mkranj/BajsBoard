...
ukupno_koristenja_po_danu = np.sum(locations_changes["total_changes"])
ukupno_postaja = locations_changes.shape[0]
ukupno_mjesta_za_bicikle = np.sum(locations_changes["no_racks"])

dnevni_prosjek_koristenja = ukupno_koristenja_po_danu / no_days
...
Na temelju dohvata javno dostupnih podataka o stanju Bajs stanica u Zagrebu provedena je analiza kako bi se utvrdili obrasci o korištenju Bajs bicikala. Podaci su prikupljeni u razdoblju od `{python} first_date.strftime("%d.%m.%Y.")` do `{python} last_date.strftime("%d.%m.%Y.")`, što obuhvaća ukupno `{python} no_days` dana.  
U trenutku provođenja analize u Zagrebu je bilo postavljeno `{python} int(ukupno_postaja)` postaja. U ovom dokumentu prikazat će se podaci o ukupnom korištenju bicikala, izdvojene stanice, te interaktivna karta s pregledom popularnosti stanica te opaženih trendova korištenja kroz sate u danu i dane u tjednu.
...
_ = plot_hourly_bar(po_satu, changes_type = "total")
```
...
# Ograničenja
Podaci za ovaj izvještaj dohvaćaju se na razini jednog sata. Zbog toga se ne bilježe apsolutne sve promjene u lokaciji bicikala unutar pojedinog sata. Drugim riječima, stvaran broj korištenja bicikala je vjerojatno **veći** od onog navedenog u ovom dokumentu.  
Ipak, osnovna namjena ove analize je prikazati trendove korištenja, a oni su vrlo vjerojatno postojani čak i ako se podaci ne dohvaćaju u realnom vremenu. Informacije o tome koje su stanice najviše i najmanje korištene, koji sati ili dani u tjednu pokazuju najveću aktivnost vrlo vjerojatno ne bi bile drugačije uz veću razinu detalja pri prikupljanju podataka.  

Ono što se mijenja je apsolutni broj korištenja: Možda za neku stanicu ovako zabilježimo da se koristi 100 puta dnevno, a stvarno stanje je 130 jer je u nekim satima isti bicikl dovezen na stanicu i odmah preuzet i odvezen dalje.  
Ova pristranost je manja za manje korištene stanice: Ako su veći broj sati zaredom opaženi isti bicikli na istoj stanici znači da tu nije došlo do nikakvih promjena, te bi to vrijedilo jednako i da se podaci dohvaćaju mnogo češće od jednom na sat.  
